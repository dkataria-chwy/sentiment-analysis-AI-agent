...
export default function ResultsDashboard({ data, summaryLoading, summaryError }: Props) {
  ...
  return (
    <>
      {/* Action Buttons - above the card, top right on desktop, centered on mobile */}
      <div className="flex flex-col items-end md:flex-row md:justify-end md:items-center w-full max-w-7xl mx-auto mt-12 mb-2 gap-2 md:gap-4 px-4">
        <button
          ...
          onClick={async () => {
            ...
            let originalDisplay;
            if (feedback) {
              originalDisplay = feedback.style.display;
            ...
            if (feedback) {
              feedback.style.display = originalDisplay;
            }
          }}
      ...
      <div className="relative max-w-7xl mx-auto mb-24 p-8 bg-white/80 backdrop-blur-md rounded-2xl shadow-2xl border-2 border-blue-300/60 flex flex-col overflow-hidden">
        ...
        {data?.product_info && (
          <div className="mb-4">
            <a
              href={data.product_info.product_link}
              target="_blank"
              rel="noopener noreferrer"
              className="product-header-link"
            >
              {data.product_info.product_name ? data.product_info.product_name : data.sku}
            </a>
            <div className="product-header-meta">
              {(() => {
                const displayParts = [];
                const labelParts = [];
                if (data.product_info.mc1) { displayParts.push(data.product_info.mc1); labelParts.push('1'); }
                if (data.product_info.mc2) { displayParts.push(data.product_info.mc2); labelParts.push('2'); }
                if (data.product_info.mc3) { displayParts.push(data.product_info.mc3); labelParts.push('3'); }
                displayParts.push(data.sku); labelParts.push('SKU');
                return <>{displayParts.join(' - ')} <span style={{color:'#b0b0b5', fontSize:'1em'}}>(MC{labelParts.join('-')})</span></>;
              })()}
            </div>
            <style jsx>{`
              .product-header-link {
        ...
        <div className="pl-6" id="results-dashboard-card">
          ...
          {/* Keyword Chips - moved below viz cards, above executive summary */}
          ...
          <div id="feedback-section" className="no-print">
            ...
            <div className="mt-8">
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full border border-blue-200 rounded-xl bg-white">
                  ...
                  <tbody>
                    {aspectSummary.map(a => {
                      // Determine dominant sentiment for color and sample reviews
                      const dominantSentiment = a.negative > a.positive ? 'negative' : 'positive';
                      const samples = a.sample_reviews && a.sample_reviews[dominantSentiment] ? a.sample_reviews[dominantSentiment] : [];
                      return (
