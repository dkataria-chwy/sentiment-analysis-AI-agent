...
export default function ProcessingDashboard({ jobId }: Props) {
  ...
  const [noDataSku, setNoDataSku] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    if (status === 'complete' || status === 'cancelled' || noData) return;
    const interval = setInterval(async () => {
      try {
        ...
        if (data.status === 'no_data') {
          // Try to get SKU from backend result if available
          if (data.result && data.result.sku) {
            setNoDataSku(data.result.sku);
          } else {
            setNoDataSku(jobId);
          }
          setNoData(true);
    ...
    <>
      {noData && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
          <div className="bg-white p-8 rounded-2xl shadow-2xl text-center max-w-sm border border-blue-200">
            <div className="text-2xl font-extrabold mb-3 text-blue-700" style={{ fontFamily: 'Inter, Segoe UI, Arial, sans-serif' }}>
              No data fetched for the entered SKU
            </div>
            {noDataSku && (
              <div className="text-lg font-mono text-gray-400 mb-6 break-all">{noDataSku}</div>
            )}
            <button
